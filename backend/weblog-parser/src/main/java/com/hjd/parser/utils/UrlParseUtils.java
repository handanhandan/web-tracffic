...
import static com.hjd.parser.utils.ParserUtils.isNullOrEmptyOrDash;

public class UrlParseUtils {

    /**
     *  对一个已经编码的字符串进行解码,在任意一个浏览器界面，检查-> console -> decodeURIComponent("https%3A%2F%2Fwww.underarmour.cn%2F%3Futm_source%3Dbaidu%26utm_term%3D%25E6%25A0%2587%25E9%25A2%2598%26utm_medium%3DBrandZonePC%26utm_channel%3DSEM")
     ...
     * 解析字符串的url为UrlInfo
     * 正常规则的url：
     * https://www.underarmour.cn/s-HOVR?qf=11-149&pf=&sortStr=&nav=640#NewLaunch  ？在#前面
     * https://www.underarmour.cn/s-HOVR#NewLaunch?qf=11-149&pf=&sortStr=&nav=640  ？在#后面
     * @param url
     * @return
     */
    public static UrlInfo getInfoFromUrl(String url) {
        String trimedUrl = url.trim(); // 左右去空
        if (isNullOrEmptyOrDash(trimedUrl)) {
            ...
            try { // 处理规则日志数据
                ...
                String query;
                String fragment;
                if (firstPoundMarkIndex > 0 && firstQuestionMarkIndex > firstPoundMarkIndex) { // #在 ？前面
            ...
            } catch (URISyntaxException e) { // 处理不规则日志数据
                try {
                    if (firstQuestionMarkIndex == -1) { // 没有？的日志数据
    ...
    private static UrlInfo parseUrlWithQuery(String url, int firstQuestionMarkIndex, int firstPoundMarkIndex) {
        QueryAndFragment queryAndFragment = getQueryAndFragment(url, firstQuestionMarkIndex, firstPoundMarkIndex); //获取query和fragment
        String urlWithoutQuery = url.substring(0, firstQuestionMarkIndex); // 解析除去query的数据
    ...
    private static UrlInfo parseUrlWithoutQuery(String trimedUrl, int firstPoundMarkIndex, boolean hasQuestionMark) {
        ...
        int colonIndex =decoderUrl.indexOf(":"); // 获取冒号的位置
